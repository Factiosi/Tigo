...
from src.core.debug_log import debug, error, warn
...
DEFAULT_EXCLUSIONS: list[str] = [
    ...
    "platform interface", "ip-https", "iphttps", "miniport", "отладчик",
    "debugger", "debug", "direct virtual", "pseudo", "lightweight filter",
    "npcap", "wintun", "hyper-v", "vethernet",
]
...
def get_adapters_info_native() -> List[Dict]:
    ...
    if result != ERROR_BUFFER_OVERFLOW:
        if result != ERROR_SUCCESS:
            _log(f"GetAdaptersInfo failed: {result}", "ERROR")
            return []
    ...
    while current:
        ...
        try:
            ...
            _log(f"Error parsing adapter: {e}", "DEBUG")
...
def get_interface_guid_from_name(adapter_name: str) -> Optional[str]:
    ...
    try:
        ...
        _log(f"Error getting GUID for {adapter_name}: {e}", "DEBUG")
    
    return None


def get_interface_guid_from_driver_desc(description: str) -> Optional[str]:
    """Resolve interface GUID by adapter DriverDesc from the class registry."""
    class_base = r"SYSTEM\CurrentControlSet\Control\Class\{4D36E972-E325-11CE-BFC1-08002BE10318}"
    target = _normalize_alias(description).lower()
    if not target:
        return None
    try:
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, class_base) as class_key:
            index = 0
            while True:
                try:
                    sub = winreg.EnumKey(class_key, index)
                    index += 1
                except OSError:
                    break
                if sub.lower() == "properties":
                    continue
                try:
                    with winreg.OpenKey(class_key, sub) as sk:
                        try:
                            ...
                            continue
                        if _normalize_alias(str(driver_desc)).lower() != target:
                            continue
                        try:
                            netcfg, _ = winreg.QueryValueEx(sk, "NetCfgInstanceId")
                        except OSError:
                            continue
                        netcfg = str(netcfg or "").strip()
                        if netcfg.startswith("{"):
                            return netcfg
                except OSError:
                    continue
    except OSError as exc:
        _log(f"Error getting GUID by DriverDesc for {description}: {exc}", "DEBUG")
    return None


def list_registry_network_connections() -> List[Tuple[str, str]]:
    """Return (connection_name, guid) for adapters registered in the network class."""
    reg_path = r"SYSTEM\CurrentControlSet\Control\Network\{4D36E972-E325-11CE-BFC1-08002BE10318}"
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, reg_path) as network_key:
            ...
            while True:
                try:
                    guid = winreg.EnumKey(network_key, index)
                    index += 1
                except OSError:
                    break
                if not guid.startswith("{"):
                ...
                try:
                    ...
                    with winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE, conn_path) as conn_key:
                        name, _ = winreg.QueryValueEx(conn_key, "Name")
                        name = _normalize_alias(str(name))
                        if name:
                            results.append((name, guid))
                except OSError:
                    continue
    except OSError as exc:
        _log(f"Error listing registry network connections: {exc}", "DEBUG")
    return results
...
def notify_dns_change():
    ...
    try:
        ...
        _log(f"Error notifying DNS change: {e}", "DEBUG")
        return False
...
def flush_dns_cache_native() -> bool:
    ...
    try:
        ...
        _log(f"Error flushing DNS cache: {e}", "DEBUG")
        return False
...
def set_doh_for_adapter(guid: str, dns_ip: str, enable: bool = True, 
    ...
    if enable and not template:
        _log(f"No DoH template for {dns_ip}", "WARNING")
        return False
...
class DNSManager:
    ...
    @staticmethod
    def is_registry_dns_adapter(name: str) -> bool:
        """Filter virtual/hidden adapters from registry connection names."""
        clean = _normalize_alias(name)
        if not clean:
            return False
        if DNSManager.should_ignore_adapter(clean, clean):
            return False
        lowered = clean.lower()
        if "*" in clean:
            return False
    ...
    def get_network_adapters_fast(
        ...
        if self.wmi_conn:
            try:
                for adapter in self.wmi_conn.Win32_NetworkAdapter(PhysicalAdapter=True):
                    ...
                    guid = get_interface_guid_from_name(adapter.NetConnectionID)
                    if not guid:
                        guid = get_interface_guid_from_driver_desc(desc)
                    if guid:
                        self._guid_cache[_normalize_alias(adapter.NetConnectionID)] = guid
                        self._guid_cache[_normalize_alias(desc)] = guid
                    
                return adapters
                
            except Exception as e:
                _log(f"WMI error: {e}", "DEBUG")
        
        # Fallback: registry connection names, then native API descriptions.
        registry_adapters = list_registry_network_connections()
        if registry_adapters:
            for name, guid in registry_adapters:
                if not include_ignored and not self.is_registry_dns_adapter(name):
                    continue
                norm = _normalize_alias(name)
                self._guid_cache[norm] = guid
        ...
        try:
            ...
            for adapter in native_adapters:
                ...
                guid = get_interface_guid_from_driver_desc(name)
                if guid:
                    self._guid_cache[_normalize_alias(name)] = guid
                
                adapters.append((name, name))
                
        except Exception as e:
            _log(f"Native API error: {e}", "ERROR")
    ...
    def get_adapter_guid(self, adapter_name: str) -> Optional[str]:
        ...
        if not guid:
            guid = get_interface_guid_from_driver_desc(adapter_name)
    ...
    def get_current_dns(self, adapter_name: str, address_family: str = "IPv4") -> List[str]:
        ...
        try:
            ...
            if not guid:
                _log(f"GUID not found for {adapter_name}", "DEBUG")
                return []
            ...
            _log(f"Error getting DNS for {adapter_name}: {e}", "DEBUG")
            return []
